...
def search_documents_by_file_name(index, query_embedding_tuple, file_name, top_k=5, include_metadata=True):
    ...
    response = query_pinecone(index, query_embedding_tuple, top_k=top_k,
                              filter_dict=file_name_filter, include_metadata=include_metadata)
...
def query_pinecone(index, query_embedding_tuple, top_k=5, filter_dict=None, include_metadata=True):
    ...
    response = index.query(query_embedding, top_k=top_k,
                           filter=filter_dict, include_metadata=include_metadata)
    return response
